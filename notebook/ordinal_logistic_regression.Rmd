...
load("regm_tidy.rda")  
  ...
  xlab("Microbiome partition\nordered by branch and by alpha-diversity") + ylab("") -> fig4A
    fig4A
...
```{r fig.height=15, fig.width=25}
  ...
  filter(s>0) %>%
  ungroup() %>%
  mutate(y.level = y.level %>% forcats::fct_reorder(shannon_median, .desc = TRUE)) %>%
  ...
  ggplot() + 
    ...
    cowplot::theme_cowplot() +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    xlab("Microbiome partition\nordered by branch and by alpha-diversity") + ylab("DMM community types") -> fig4A
    fig4A
  #ggsave("partitions_multinomial_logOR.pdf", height = 12, width = 14)
...
regm_tidy %>%
 ...
 regm_tidy %>%
  ...
  facet_grid(branch~., scale="free", space = "free") + scale_fill_brewer("",type="qual", palette = "Set2") +
  ylab("") + xlab("absolute cummulated Log OR") + 
   cowplot::theme_cowplot() -> fig4C


 (fig4B + plot_spacer() + plot_layout(widths = c(3,1.22))) /
   (fig4A + fig4C + plot_layout(widths = c(3,1))) + plot_layout(heights = c(1,3)) 
