...
metadata %>%
  ...
  group_by(Enterotypes_id_before) %>%
  mutate(p=n/sum(n)) %>%
  dplyr::slice(1:3) -> enterotypes_time_series_network
...
ggraph(graph, weights=p, layout="stress") +
  ...
  scale_edge_alpha("ind. proportion") + 
  scale_size_continuous("time stability") +
  theme_void() -> fig5A
...
df %>%
  ...
  xlim(-4,4) + ylim(-0.5, 0.5) + cowplot::theme_cowplot() + 
  xlab("Feacalibacterium/Bacteroides variation") + ylab("alpha-diversity variation")
