...
genus_phate$embedding %>%
  ...
  scale_color_viridis_c("Shannon", option="cividis") + cowplot::theme_cowplot() +
  ggrepel::geom_label_repel(data = enterotypes_phate_centroid, aes(x=PHATE1, y=PHATE2, label=Enterotypes_id)) -> fig1E
...
M %>%  
  ...
  scale_color_gradient("Nb of\nsamples") +
    geom_node_text(aes(label = name), colour = 'white', vjust = 0.4) + 
  cowplot::theme_cowplot() + xlab("PHATE 1") + ylab("PHATE 2") -> fig1D
...
enterotypes %>%
  ...
  xlab("DMM community types") +
  cowplot::theme_cowplot() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) -> fig1B
