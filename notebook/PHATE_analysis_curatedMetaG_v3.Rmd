...
genus_phate$embedding %>%
  ...
  mutate(branch = ifelse(Enterotypes_id %in% c(2,3,8,9,14,15,16), "Prevotella DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c(4,5,13,18), "Bacteroides DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c(17,21,22,23,24), "Bifidobacterium DMM types",branch)) %>%
  mutate(branch = ifelse(Enterotypes_id %in% c(19,21,24), "Escherichia DMM types",branch)) %>%
